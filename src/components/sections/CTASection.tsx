            ...
            <div className="max-w-7xl mx-auto relative">
                ...
                <div className="text-center mb-16">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
                        <Link
                            to="/app"
                            className="group cursor-pointer relative px-8 py-4 rounded-xl text-lg font-semibold bg-gradient-to-r from-green-500 to-blue-500 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500/20 no-underline"
                        >
                            {/* Background glow */}
                        ...
                        <Link
                            to="/partner"
                            className="group cursor-pointer relative px-8 py-4 rounded-xl text-lg font-semibold backdrop-blur-sm border border-green-200/60 dark:border-green-700/60 hover:border-green-300/80 dark:hover:border-green-600/80 transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500/20 no-underline"
                        >
                            {/* Background */}
                            ...
                            {/* Content */}
                            <div className="relative flex items-center justify-center space-x-3">
                                <Handshake className="w-6 h-6 group-hover:text-green-600 dark:group-hover:text-green-400 transition-all duration-300 group-hover:animate-bounce" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {[
                        ...
                        },
                        {
                            ...
                            route: '/business-program'
                        }
                    ].map((item, index) => (
                        <div
                            key={index}
                            className="relative text-center backdrop-blur-sm rounded-xl p-8 border border-green-200/60 dark:border-green-700/60 hover:border-green-300/80 dark:hover:border-green-600/80 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:shadow-green-500/10"
                        >
                            {/* Background gradient on hover */}
                            <div className={`absolute inset-0 bg-gradient-to-br ${item.color} opacity-0 hover:opacity-10 rounded-xl transition-opacity duration-500 pointer-events-none`}></div>

                            {/* Icon com animação especial para handshake */}
                            <div className={`inline-flex items-center justify-center w-16 h-16 rounded-lg bg-gradient-to-br ${item.color} text-white mb-6 hover:scale-110 transition-transform duration-300`}>
                                {index === 1 ? (
                            ...
                            {/* Action Button - Link funcional */}
                            <Link
                                to={item.route}
                                className={`w-full block px-4 py-3 rounded-lg bg-gradient-to-r ${item.color} text-white font-semibold transition-all duration-300 hover:scale-105 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 text-center no-underline relative z-10`}
                            >
                                Saiba Mais
                            </Link>

                            {/* Hover effect line */}
                            <div className={`absolute bottom-0 left-0 h-1 bg-gradient-to-r ${item.color} transform scale-x-0 hover:scale-x-100 transition-transform duration-500 rounded-b-xl pointer-events-none`}></div>
                        </div>
                    ))}
                </div>
